...
from .context_graph import ScreenContext

def _rewrite(string: str, rules: dict, times: int)->str: #Functional style? using recursion
...
def draw(
    ...
    with ScreenContext() as context:
        ...
        myturtle.goto(0, 0)
        myturtle.pendown()
        turtle_path = _string2turtle(
            _rewrite(string=string, rules=rules, times=deepth),
            myturtle=myturtle,
        )
